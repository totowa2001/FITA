// FaucetHintManager.cs
// 251202 Update ver.
// 251201_2 Scene에 쓰이는 버전임.
// 251127 및 그 이전에 사용을 위해 rollback 시 Header 부분 표시돼있는 주석 사이를 지울 것
...
public class FaucetHintManager : MonoBehaviour
{
    ...
    // 251201 추가된 부분
    // rollback 시 이부분 삭제할 것
    [Tooltip("Environment Depth에서 픽셀별 깊이를 읽어오는 샘플러")]
    public EnvironmentDepthSamplerForFITA depthSampler;
    // 여기까지 251201
    // rollback 시 이 주석 사이를 삭제할 것
    ...
    public void OnYoloDetections(List<Det> dets, int frameWidth, int frameHeight)
    {
        ...
        // 여기서부터는 1127 이전으로 rollback 시 사용
        ...
        // 4-A) 우선 DepthSampler로 깊이 값을 얻어본다
        bool placedWithDepth = false;

        if (depthSampler != null && cameraAccess != null)
        {
            if (depthSampler.TryGetDepthMeters(viewportUV, out float depthMeters))
            {
                // 카메라에서 나온 Ray
                Ray depthRay = cameraAccess.ViewportPointToRay(viewportUV);

                // depthMeters 만큼 나간 위치 (대략 월드 좌표)
                Vector3 hitPosApprox = depthRay.origin + depthRay.direction * depthMeters;

                // 표면에서 살짝 띄우기 (ray 방향 반대쪽으로 offset)
                Vector3 targetPos = hitPosApprox - depthRay.direction * surfaceOffset;
                ...
                {
                    // 깊이만으로는 법선 정보를 모르기 때문에, faceCamera=false 인 경우엔
                    // 일단 카메라 방향을 반대로 보는 정도로만 처리
                    _hintInstance.rotation = Quaternion.LookRotation(-depthRay.direction);
                }

                _hintInstance.gameObject.SetActive(true);
                placedWithDepth = true;

                // 디버그용
                Debug.DrawRay(depthRay.origin, depthRay.direction * depthMeters, Color.cyan);
            }
        }

        // 4-B) 만약 Depth 기반으로 못 놓았으면, 기존 Mesh Raycast 시도 (있다면)
        if (!placedWithDepth)
        {
            Ray ray = cameraAccess.ViewportPointToRay(viewportUV);
            RaycastHit hit;
        ...
        // 여기서부터 다시 1127 버전
        // 1127 이전으로 rollback 시 아래 코드 살리기

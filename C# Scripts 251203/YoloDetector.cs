...
public class YoloDetector : MonoBehaviour {
    ...
    public FaucetHintManager faucetHintController; // Faucet 탐지 시 3D 오브젝트 띄우는 코드와 연결
    ...
    public void RunDetection(Texture currentFrameTexture) {
        ...
        // 4. 시각화 스크립트, 3D 오브젝트 스크립트(faucetHintController)로 전달
...
List<Det> Decode(Tensor o, int origW, int origH)
{
    ...
    float scaleX = (float)origW / inputSize;  // inputSize는 640
